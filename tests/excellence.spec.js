...
test.describe('DeePonyCap excellence v2.7', () => {
  ...
  test('fuzzy search finds pony by partial name', async ({ page }) => {
    ...
    await page.locator('#tab-collection .search').fill('twil');
    await page.waitForTimeout(300);
    await expect(page.locator('#tab-collection').getByText('Twilight Sparkle').first()).toBeVisible({ timeout: 5000 });
  });
  ...
  test('pony passport opens from collection card', async ({ page }) => {
    await page.locator('[data-tab="collection"]').click();
    await page.locator('#tab-collection .pony-card').first().click();
